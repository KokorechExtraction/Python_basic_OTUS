...
def _input_validation(msg, error_msg):
    while True:
...
def input_check(data):
    ...
    if data == 'name':
        ...
        while  len(number) != 10 or not number.isdigit():
            number = input('Номер должен содержать не более десяти цифр. Повторно введите номер или введите 8 для возврата в меню: ')
            if number == '8':
                return '8'
        return number
    elif data == 'comment':
